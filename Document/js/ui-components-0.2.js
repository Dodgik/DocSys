(function (window, $, undefined) {
    ...
    function commentsBlock(o) {
        ...
        $('<input type="button" value="Надіслати"/>').button().click(function () {
            var urlRequest = appSettings.rootUrl + 'Handlers/DataPoint.ashx?obj=coms&type=ins' + '&dep=' + userdata.departmentId + '&documentId=' + o.DocumentID;
            urlRequest += (o.ControlCardID ? '&controlCardId=' + o.ControlCardID : '');
        ...
        $('<input type="button" value="Виконано"/>').button().click(function () {
            var urlRequest = appSettings.rootUrl + 'Handlers/DataPoint.ashx?obj=coms&type=ins' + '&dep=' + userdata.departmentId + '&documentId=' + o.DocumentID;
            urlRequest += (o.ControlCardID ? '&attachToCard=1&controlCardId=' + o.ControlCardID : '');
            var actionComment = 'Виконано';
            $.ajax({
                url: urlRequest,
                type: 'POST',
                cache: false,
                data: { 'data': actionComment },
                dataType: 'json',
                success: function (msg) {
                    updateComments();
                },
                ...
                success: function (response) {
                    if (response.Data) {
                        ...
                        response.Data.forEach(function (c) {
                            var commentItem = $('<div class="comment-item' + (o.selectedCommentId == c.ID ? ' selected' : '') + '" comment-id="' + c.ID + '"></div>').appendTo(cBlock);
                            var commentTitle = $('<span class="comment-title"></span>').appendTo(commentItem);
                            var cDate = new Date(+c.CreateDate.replace(/\/Date\((-?\d+)\)\//gi, '$1'));
                            $('<span class="comment-time">' + formatDateTime(cDate) + ' </span>').appendTo(commentTitle);
                            var workerName = documentUI.formatFullName(c.Worker.LastName, c.Worker.FirstName, c.Worker.MiddleName);
