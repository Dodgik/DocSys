(function (window, $, undefined) {
    ...
    var workerStatements = function (selector, options) {
        var self = this;
        ...
        this.init = function () {
            ...
            $('#dg' + selector).jqGrid({
                ...
                colNames: ['№', 'documentID', 'Дата', 'Прізвище', 'fullName', 'Адреса', 'Керівник', 'Номер', 'Номер в орг.',
                    'Зміст', 'DeliveryTypeID', 'inputDocTypeID', 'inputMethodID', 'inputSignID', 'inputSubjectTypeID', 'ControlCardID', 'OpenWorker', 'К.', 'Контроль', 'controlEndDate'],
                colModel: [
                    ...
                    {
                        name: 'OpenWorker', width: 0, sortable: false, hidden: true, formatter: function (cellValue, cOptions, rowObject) {
                            if (rowObject[18].toLowerCase() == 'true') {
                                self.rowsToColor.push(cOptions.rowId);
                            }
                            if (rowObject[16].toLowerCase() !== 'true') {
                                self.rowsToColorOpenWorker.push(cOptions.rowId);
                            }
                            var controlEndDate = rowObject[19];
                            if (controlEndDate) {
        ...
        this.showUpdateForm = function () {
            ...
            if (rowId) {
                ...
                var urlRequest = appSettings.rootUrl + 'Handlers/DataPoint.ashx?obj=wdocs&type=getstblank&jdata=' + rowObj.id;
    ...
    window.WorkerStatements = workerStatements;
})(window, jQuery);
