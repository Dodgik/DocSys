(function (window) {
    ...
    var documentsTable = function (selector, options) {
        var self = this,
            tpls = window.docTemplates,
            tplName = options.templateName || 'documentBlank';
        ...
        this.init = function () {
            ...
            $(this.appendTo).append($('<table></table>').attr('id', selector)
                    ...
                    .append($("<table></table>").attr('id', "tblOverview" + selector)
                        ...
                        .append($('<tr><td style="background-color: #F0F0F0">№ рішення:</td></tr>').append(createDetailCell($('<td></td>'))))
                        .append($('<tr><td style="background-color: #F0F0F0">Дата:</td></tr>').append(createDetailCell($('<td></td>'))))
                        .append($('<tr><td style="background-color: #F0F0F0">Назва рішення:</td></tr>').append(createDetailCell($('<td></td>'))))
                        .append($('<tr><td style="background-color: #F0F0F0">Примітки:</td></tr>').append(createDetailCell($('<td></td>'))))
                            ))
            ...
            $('#dg' + selector).jqGrid({
                ...
                mtype: 'POST',
                height: '550',
                colNames: ['№', 'documentID', 'Дата', '№ рішення', '№ в організації', 'Назва рішення', '№ сесії', 'Примітки', 'Контроль',
                ...
                colModel: [
                    ...
                    { name: 'number', index: 'Number', width: 100, sortable: true },
                    { name: 'externalNumber', index: 'ExternalNumber', width: 190, sortable: false, hidden: true },
                    { name: 'content', width: 390, sortable: false, hidden: false },
                    { name: 'changes', width: 80, sortable: false, hidden: true },
                    ...
                    { name: 'organizationName', index: 'OrganizationName', width: 360, sortable: false, hidden: true }
                ],
                rownumbers: true,
                rowNum: 100,
                ...
                onSelectRow: function (rowid) {
                    if (rowid) {
                        ...
                        self.detailsCells[3].text(rowObj.notes);
                    }
                },
                ...
                subGridRowExpanded: function (subGridId, rowId) {
                    ...
                    $("#" + subgridTableId).jqGrid({
                        ...
                        ondblClickRow: function (rowid, iRow, iCol, e) {
                            ...
                            var ccBlank = new CardBlankDecision({ departmentID: self.departmentID });
                    ...
                    .navButtonAdd("#" + pagerId, {
                        ...
                        onClickButton: function () {
                            var ccBlank = new CardBlankDecision({ departmentID: self.departmentID, documentID: rowObj.documentID });
