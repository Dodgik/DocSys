...
namespace BizObj.Data
{
	...
	public sealed class SPHelper
	{
        ...
        {
            Exception customException = new Exception("System error! Error in calling database functions. Exception Message: " + ex.Message);
	...
	public sealed class SpHelperParameterCache
	{
        ...
        public static void CacheParameterSet(string commandText, params SqlParameter[] commandParameters)
		{
            try
            {
                SqlHelperParameterCache.CacheParameterSet(GetConnectionString(), commandText, commandParameters);
            }
            catch (Exception ex)
            {
                ThrowException(ex);
