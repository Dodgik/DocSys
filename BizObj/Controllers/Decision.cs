...
namespace BizObj.Controllers
{
    public class Decision
    {
        ...
        public Decision(HttpContext context, Worker worker)
        {
            _context = context;
            Worker = worker;
        }
        public Decision(HttpContext context, Worker worker, string userName)
        {
            _context = context;
            Worker = worker;
            UserName = userName;
        ...
        private bool IsValidDocTemplate(DocTemplate docTemplate, out string msg)
        {
            ...
            if (docTemplate.Document.ExternalSource != null && String.IsNullOrWhiteSpace(docTemplate.Document.ExternalSource.Number) && docTemplate.IsInput)
            {
                msg = "Не вказаний номер документа в організації";
                return false;
            }
            DateTime dscd = docTemplate.Document.ExternalSource.CreationDate;
            if (docTemplate.Document.ExternalSource != null && (dscd < (DateTime)SqlDateTime.MinValue || dscd > (DateTime)SqlDateTime.MaxValue) && docTemplate.IsInput)
            {
        ...
        private int InsertDocTemplateAsync(DocTemplate docTemplate)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    DateTime dscd = docTemplate.Document.ExternalSource.CreationDate;
                    if (docTemplate.Document.ExternalSource != null && (dscd < (DateTime)SqlDateTime.MinValue || dscd > (DateTime)SqlDateTime.MaxValue))
                    {
                        docTemplate.Document.ExternalSource.CreationDate = (DateTime)SqlDateTime.MinValue;
                    }

                    if (docTemplate.Document.ExternalSource.OrganizationID > 0)
                    {
                        docTemplate.Document.ExternalSource.UserName = UserName;
                        docTemplate.Document.ExternalSource.ID = docTemplate.Document.ID;
                        docTemplate.Document.ExternalSource.Insert(trans);
                    }
                    else
                    {
                        if (!String.IsNullOrWhiteSpace(docTemplate.Document.ExternalSource.OrganizationName))
                        {
                            Organization org = new Organization(UserName);
                            org.Name = docTemplate.Document.ExternalSource.OrganizationName;
                            org.OrganizationTypeID = 2;
                            org.DepartmentID = Worker.DepartmentID;
                            org.WorkerID = Worker.ID;
                            org.Insert(trans);
                            docTemplate.Document.ExternalSource.OrganizationID = org.ID;
                        }
                        if (!String.IsNullOrWhiteSpace(docTemplate.Document.ExternalSource.OrganizationName) || !String.IsNullOrWhiteSpace(docTemplate.Document.ExternalSource.Number))
                        {
                            docTemplate.Document.ExternalSource.UserName = UserName;
                            docTemplate.Document.ExternalSource.ID = docTemplate.Document.ID;
                            docTemplate.Document.ExternalSource.Insert(trans);
        ...
        private ProcessingResult UpdateDocTemplateAsync(DocTemplate docTemplate)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        docTemplate.Document.ExternalSource.UserName = UserName;
                        if (docTemplate.Document.ExternalSource.ID > 0)
                            docTemplate.Document.ExternalSource.Update(trans);
                        else
                        {
                            docTemplate.Document.ExternalSource.ID = docTemplate.Document.ID;
                            docTemplate.Document.ExternalSource.Insert(trans);
                    ...
                    else
                    {
                        ...
                        else if (ExternalSource.IsExist(trans, docTemplate.DocumentID))
                        {
                            ExternalSource.Delete(trans, docTemplate.DocumentID, UserName);
                        }
                    }
        ...
        public void GetDocTStatisticsReport(int departmentID, DateTime startDate, DateTime endDate, int documentCodeID)
        {
            ...
            int sumCountDocs = 0,
                sumControlledDocs = 0;
            string[] cols = { "OrganizationID", "OrganizationName" };
            DataTable orgsTable = ds.Tables[1].DefaultView.ToTable(true, cols);
            foreach (DataRow org in orgsTable.Rows)
            {
                ds.Tables[1].DefaultView.RowFilter = String.Format("OrganizationID = {0}", org["OrganizationID"]);

                List<string> row = new List<string> { (string)org["OrganizationName"], String.Empty, String.Empty };
        ...
        public void GetDocTStatisticsReportDocumentsVolume(int departmentID, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                RtfTable tbl = new RtfTable(RtfTableAlign.Center, 4, 8);
